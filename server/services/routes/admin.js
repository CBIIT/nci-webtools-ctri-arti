...
import {
  eq, and, or, between, like, sql, sum, count, countDistinct, asc, desc,
} from "drizzle-orm";
...
api.get("/admin/users/:id/usage", requireRole("admin"), routeHandler(async (req, res) => {
  ...
  const where = and(
    ...
    between(Usage.createdAt, startDate, endDate),
  );
...
api.get("/admin/usage", requireRole("admin"), routeHandler(async (req, res) => {
  ...
  const rows = await db.query.Usage.findMany({
    ...
    with: {
      ...
      User: {
        columns: { id: true, email: true, firstName: true, lastName: true, budget: true, remaining: true },
...
api.get("/admin/analytics", requireRole("admin"), routeHandler(async (req, res) => {
  ...
  if (groupBy === "user") {
    ...
    if (statusFilter && statusFilter !== "All") joinConditions.push(eq(User.status, statusFilter));
    ...
    const data = await db
      ...
      .groupBy(
        ...
        User.budget, User.remaining, User.roleID,
        Role.id, Role.name,
      )
      .orderBy(orderFn(orderCol))
      .limit(limit)
