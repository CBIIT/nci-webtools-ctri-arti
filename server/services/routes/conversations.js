...
api.put(
  ...
  routeHandler(async (req, res) => {
    ...
    if (existingAgent.creatorID === null) {
...
api.post(
  ...
  routeHandler(async (req, res) => {
    const userId = req.session.user.id;
    const vectors = await cmsClient.addVectors(userId, req.body.vectors);
...
api.get(
  ...
  routeHandler(async (req, res) => {
    const userId = req.session.user.id;
    const vectors = await cmsClient.getVectorsByResource(userId, req.params.resourceId);
    res.json(vectors);
  })
);
