...
import {
  deleteGuardrailById,
  listGuardrails,
  toRuntimeGuardrailConfig,
} from "gateway/core/guardrails.js";
...
test("guardrail seed data is available to gateway management", async () => {
  ...
  const fedPulseGuardrail = guardrails.find(
    (guardrail) => guardrail.name === "FedPulse Prompt Attack"
  );
...
test("deleteGuardrailById refuses to delete assigned guardrails", async () => {
  const [fedPulseGuardrail] = await db.select().from(Guardrail).where(eq(Guardrail.id, 1)).limit(1);
  assert.ok(fedPulseGuardrail, "FedPulse guardrail should exist");

  await assert.rejects(
    () => deleteGuardrailById(fedPulseGuardrail.id),
    (error) => {
