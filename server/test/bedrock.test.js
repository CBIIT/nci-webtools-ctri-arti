...
test.skip('BedrockProvider', async (t) => {
  ...
  await t.test('converse method should return valid response structure', async () => {
    ...
    assert.strictEqual(response.usage.totalTokens, response.usage.inputTokens + response.usage.outputTokens);
  ...
  await t.test('should handle token usage tracking', async () => {
    ...
    const longInput = {
      ...
      messages: [{ 
        role: 'user', 
        content: [{ text: 'Tell me about artificial intelligence and machine learning technologies' }] 
      }],
  ...
  await t.test('Context Caching', async (cacheTests) => {
    ...
    const generateLargeContent = (baseText = 'This is a comprehensive analysis of artificial intelligence and machine learning technologies in modern computing systems.') => {
      // Repeat content to ensure we exceed the 1024 token minimum for caching
      return Array(50).fill(baseText).join(' ') + ' ' + 
    ...
    await cacheTests.test('should handle system-level cache points', async () => {
      const largeSystemPrompt = generateLargeContent('You are an expert AI assistant specializing in detailed analysis of complex technical topics.');
    ...
    await cacheTests.test('should handle cache write and read cycle', async () => {
      const largeContent = generateLargeContent('Detailed technical documentation about software architecture patterns and design principles.');
      ...
      if (firstResponse.usage.cacheWriteInputTokens !== undefined) {
        console.log('First request - Cache write tokens:', firstResponse.usage.cacheWriteInputTokens);
      }
      
      if (secondResponse.usage.cacheReadInputTokens !== undefined) {
        console.log('Second request - Cache read tokens:', secondResponse.usage.cacheReadInputTokens);
        ...
        if (secondResponse.usage.cacheReadInputTokens > 0) {
          ...
          console.log('Note: Cache read tokens are 0 - may indicate model does not support caching or content below threshold');
        }
      } else {
        console.log('Note: Cache token fields not present in response - may indicate model does not support caching');
      }
    ...
    await cacheTests.test('should handle cache miss with different content', async () => {
      ...
      const modifiedContent = generateLargeContent('Second version with different content that should not hit cache.');
      ...
      const tokenDifference = Math.abs(firstResponse.usage.inputTokens - secondResponse.usage.inputTokens);
      ...
      assert.ok(tokenDifference < 100, `Token difference (${tokenDifference}) should be within reasonable range for similar content`);
    });
    ...
    await cacheTests.test('should handle streaming with cache points', async () => {
      const largeContent = generateLargeContent('Streaming test content with cache points for verification.');
    ...
    await toolTests.test('should handle tool with complex parameters', async () => {
      ...
      const input = {
        ...
        messages: [
          {
            role: 'user',
            content: [{ text: 'Search the database for "machine learning" articles from 2023, limit to 5 results' }]
          }
        ],
        toolConfig: {
          tools: tools
    ...
    await toolTests.test('should handle multiple tools', async () => {
      ...
      const input = {
        ...
        messages: [
          {
            role: 'user',
            content: [{ text: 'I have multiple tools available. Can you list what you can help me with?' }]
          }
        ],
        toolConfig: {
          tools: tools
      ...
      const mentionsCalculation = responseText.includes('add') || responseText.includes('number') || responseText.includes('math');
    ...
    await toolTests.test('should handle streaming with tool calls', async () => {
      ...
      const input = {
        ...
        messages: [
          {
            role: 'user',
            content: [{ text: 'Please format "hello world" to uppercase using the available tool' }]
          }
        ],
        toolConfig: {
          tools: tools
