...
function createUsagePageHandler(overrideHandler) {
  return ({ url, request, originalFetch }) => {
    const overridden = overrideHandler?.({ url, request, originalFetch });
    if (overridden) {
      return overridden;
    }
    ...
    if (url.pathname === "/api/v1/admin/analytics") {
      ...
      if (groupBy === "model") {
        return jsonResponse({
          data: [{ Model: { name: "Scripted Model" }, totalRequests: 2, totalCost: 0.3 }],
        });
      }
...
test("Usage Dashboard Tests", async (t) => {
  ...
  try {
    ...
    await t.test("/_/usage has filter controls", async () => {
      ...
      const { container, errors } = page;
      try {
        await waitForElement(container, "h1", (el) =>
          el.textContent.includes("AI Usage Dashboard")
        );
    ...
    await t.test("/_/usage table exposes the key usage dashboard columns", async () => {
      const page = mountUsagePage("/_/usage");
      const { container, errors } = page;
      try {
...
test("User Usage Detail Tests", async (t) => {
  ...
  try {
    ...
    await t.test(
      "/_/users/:id/usage recent requests group by request id and model id",
      async () => {
        const usageTestUser = {
          ...testUser,
          id: 123,
        ...
        try {
          ...
          assert.ok(
            dailyHeaders.includes("Guardrail Cost"),
            "daily usage should show Guardrail Cost"
          );
