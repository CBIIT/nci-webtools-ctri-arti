...
test('Chat Message Component Tests', async (t) => {
  ...
  await t.test('renders markdown content correctly', () => {
    ...
    try {
      ...
      render(() => html`<${Message} message=${message} messages=${[message]} index=${0} />`, container);
  ...
  await t.test('opens feedback dialog when feedback button is clicked', () => {
    ...
    try {
      ...
      assert.strictEqual(dialogShown, true, 'Dialog should be shown when feedback button is clicked');
    } finally {
      document.body.removeChild(container);
    }
  });
