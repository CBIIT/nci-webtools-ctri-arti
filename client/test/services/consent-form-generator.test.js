...
async function runModel(params) {
  console.log(
    `[runModel] POST /api/model/invoke (model=${params.model}, outputConfig=${!!params.outputConfig})`
  );
...
test("Consent Form Generator", async (t) => {
  await t.test("end-to-end: filter + extract for Atezolizumab protocol", async () => {
    ...
    console.log(
      `[load] consent-library.txt: ${productionLibraryText.length} chars (production reference)`
    );
    console.log(`[load] protocol.txt: ${protocolText.length} chars`);
    console.log(`[load] prompt-v3.txt: ${promptTemplate.length} chars`);
    console.log(
      `[load] consent-schema.json: ${schemaFields.length} fields, ${fullSchema.required?.length} required`
    );
      ...
      if (value === undefined) {
        ...
        if (field === "drug_risks") {
          ...
          for (const item of value.slice(0, 3)) {
            const preview =
              typeof item === "string" ? item.slice(0, 80) : JSON.stringify(item).slice(0, 80);
            console.log(`    - ${preview}...`);
          }
    ...
    assert.ok(
      typeof filterResult.reasoning === "string" && filterResult.reasoning.length > 0,
      "Filter reasoning should be non-empty"
    );
    ...
    assert.ok(
      filterResult.matched_keys.length >= 10,
      `Should match >= 10 sections, got ${filterResult.matched_keys.length}`
    );
    assert.ok(
      filterResult.matched_keys.length <= 40,
      `Should match <= 40 sections, got ${filterResult.matched_keys.length}`
    );
    console.log(
      `[PASS] matched_keys count (${filterResult.matched_keys.length}) in range [10, 40]`
    );

    // All returned keys must exist in the library
    for (const key of filterResult.matched_keys) {
      assert.ok(
        libraryKeys.includes(key),
        `Matched key "${key}" must exist in consent-library.json`
      );
    }
    ...
    assert.ok(
      ...
      `All required fields present. Missing: ${missingFields.join(", ")}`
    );
    ...
    for (const drug of extraction.drug_risks) {
      assert.ok(
        ...
        `drug_name should be non-empty, got "${drug.drug_name}"`
      );
    ...
    assert.ok(
      extraction.study_duration === "2 years",
      `study_duration should be "2 years", got "${extraction.study_duration}"`
    );
    ...
    assert.ok(
      extraction.is_investigational === true,
      "is_investigational should be true (atezolizumab is investigational in this study)"
    );
    console.log(`[PASS] is_investigational: ${extraction.is_investigational}`);

    assert.ok(
      extraction.parent_permission === false,
      "parent_permission should be false (adults only, age >= 18)"
    );
    console.log(`[PASS] parent_permission: ${extraction.parent_permission}`);

    assert.ok(
      extraction.no_payment === true,
      "no_payment should be true (no payment mentioned in protocol)"
    );
