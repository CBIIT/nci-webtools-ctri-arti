...
export class Database2 {
        ...
        if (oldConversations.length > 0 || oldMessages.length > 0) {
          ...
          for (const oldMsg of oldMessages) {
            try {
              ...
              console.error(`Error migrating message for conversation ${oldMsg.conversationId}:`, error);
            }
          }
          
          console.log(`Migration completed: ${convMap.size} conversations and ${oldMessages.length} messages processed`);
        }
      } catch (error) {
  ...
  async deleteProject(id) {
    ...
    await Promise.all(
      projectLevelResources.map(r => this.db.delete("resources", r.id))
  ...
  async deleteConversation(id) {
    ...
    await Promise.all(
      messages.map(m => this.db.delete("messages", m.id))
