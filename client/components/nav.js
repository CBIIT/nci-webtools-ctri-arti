      ...
      <div class="container">
        ...
        <div class=${() => ["collapse navbar-collapse", expanded() ? "show" : ""].join(" ")} id="navbarNavDropdown">
          <ul class="navbar-nav me-auto">
            <${For} each=${() => routes.filter((route) => !route.hidden)}>${(route) => html`
              <li class=${["nav-item", route.children ? "dropdown" : ""].join(" ")}>
                ...
                <${Show} when=${route.children}>
                  <ul class=${() => ["dropdown-menu border-0", menuOpen()[route.path] === route.title ? "show" : ""].join(" ")}>
                    <${For} each=${route.children?.filter(c => !c.hidden)}>${(child) => html`
                      <li>
                        <${A}
          ...
          <div class="navbar-nav">
            <${Show} when=${() => session()?.user} fallback=${html`<a href="/api/login" target="_self" class="nav-link text-decoration-none">Login</a>`}>
              <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" onClick=${() => toggleMenu('nav', 'user')}  href="#" role="button" >${() => session()?.user?.firstName || 'User'}</a>
                <ul class=${() => ['dropdown-menu bg-transparent text-end end-0 border-0', menuOpen()['nav'] === 'user' ? 'show' : ''].join(' ')}>
